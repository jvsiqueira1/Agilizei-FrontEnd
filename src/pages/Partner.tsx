    ...
    <div className="min-h-screen bg-white text-black">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.filter(service => 
            ...
            >
              ...
              <p className="mb-2"><span className="font-medium">Status:</span> {getServiceStatus(service)}</p>
              <p className="mb-2"><span className="font-medium">Data:</span> {service.date}</p>
                ...
                <div>
                  ...
                  {service.budgets.map(budget => (
                    <p key={budget.id} className="text-sm">
                      {budget.professional}: R$ {budget.value ? budget.value.toFixed(2) : 'N/A'} - {budget.description}
                    </p>
                  ))}
                </div>
              )}
        ...
        <BudgetModal 
          ...
          isAccepted={selectedService ? selectedService.status === 'completed' : false} // Pass isAccepted prop
          isInactive={selectedService ? selectedService.status === 'not_chosen' : false} // Pass isInactive prop
          attachments={selectedService ? selectedService.budgets?.flatMap(b => b.attachments || []) : []} // Pass attachments, ensuring no undefined values
        />
      </main>
      <Footer />
    </div>
