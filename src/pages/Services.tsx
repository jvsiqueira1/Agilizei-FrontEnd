...
import { FaBroom, FaBolt, FaPaintBrush, FaTools, FaTree, FaTruck, FaHammer } from 'react-icons/fa'; 
import { motion } from 'framer-motion';
import { useState } from 'react';
import Modal from '../components/Modal'; 
import { Button } from '../components/ui/button'; 
import { Input } from '../components/ui/input'; 
import ClientForm from '../components/ClientForm'; // Importing ClientForm

export default function Services() {
  const { openMenu } = useMenu();
  const [clientModal, setClientModal] = useState(false); // State for modal visibility
      ...
      <section className="services grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-8">
        ...
        <div className="flex justify-center">
          <motion.div className="service-card bg-[#DB4E1E] shadow-lg p-4 rounded-lg hover:shadow-xl transition-transform transform hover:scale-105">
            ...
            <p className="text-white">Construção e reforma de ambientes, assentamento de pisos, e muito mais para transformar seu espaço!</p>
          </motion.div>
        </div>
      </section>

      <Button className="px-5 fixed right-4 bottom-12 bg-[#DB4E1E] text-white text-lg animate-pulse" onClick={() => setClientModal(true)} variant="outline">Contratar agora!</Button>
